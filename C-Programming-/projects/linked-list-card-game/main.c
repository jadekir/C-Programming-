...
// Student Name: Jade Kirkpatrick
// File Name: pa2_kirkpatrick.c
...
typedef struct LLNode_s {
    ...
    struct LLNode_s *next;  // A pointer reference to the next node, or NULL if
                            // it is the last element in the list.
} LLNode;

typedef struct LList_s {
    LLNode *head;  // A pointer to the linked list's head node, or NULL if list
                   // is empty.
} LList;

typedef struct player_s {
    char *name;            // Dynamically allocated string of the player's name.
    LList *list_of_cards;  // Pointer to a linked list that holds all of the
                           // player's cards.
} player_t;
...
typedef struct GameBoard_s {
    ...
    LList *draw_pile;  // Pointer to a linked list for the draw pile, from which
                       // cards will be dealt.
    LList *discard_pile;  // Pointer to a linked list for the discard pile,
                          // where cards will be discarded.
} GameBoard;
...
int main(void) {
    ...
    // we do this in the following sequence (order): player 1, player 2, and so
    // on we keep doing this until the board's draw pile runs out of cards
    // therefore, it is possible for some players to receive fewer number of
    // cards compared to other players
    ...
    // we do this in the following sequence (order): player 1, player 2, and so
    // on for each player, the player returns or puts one card at a time to the
    // discard pile essentially, when the player is done returning all the
    // cards, it will be in reversed order in the discard pile
    printf("Discarding All Cards.\n");
    discard_all_cards(board);

    // print the status of the game board after all players discarded their
    // cards
    print_game_status(board);
...
GameBoard *load_file(FILE *file_ptr, int *count_of_players) {
    ...
    // only for processing the input file, we will assume that the names of the
    // players will have a maximum of 254 characters; however, when these
    // players are later created, their names must be dynamically allocated
    // (i.e., strings)
    ...
    for (int i = 0; i < *count_of_players; i++) {
        ...
        // create a player; then store this in a dynamic array of pointers to
        // players
    ...
    // read from the file how many cards to process, usually 51 (but it may
    // vary)
    int card_count;
    fscanf(file_ptr, "%d\n", &card_count);
 ...
 * If it is unable to dynamically allocate the linked list at runtime, it simply
 * returns a NULL. It keeps track of the first element of the list which is
 * referred to as the head. Because the linked list is initially empty, head
...
LList *create_linked_list() {
    ...
    // check if the dynamic allocation was successful or not; if it was
    // unsuccessful, return NULL set the head pointer to NULL afterward, return
    // the linked list
    LList *linked_list = (LList *)malloc(sizeof(LList));
...
/**
 * @brief This function dynamically creates an LLNode which can be later be used
 * in a LList. If it is unable to dynamically allocate the node at runtime, it
 * simply returns a NULL. It sets the suit and the rank values of the card_t
 * card of this node based on the arguments received. It ensures that the suit
 * parameter is in upper-case.
 * @param char suit
 * @param int rank
...
player_t **create_list_of_players(int count_of_players) {
    ...
    player_t **list_of_players =
        (player_t **)malloc(count_of_players * sizeof(player_t *));
    if (!list_of_players) return NULL;
 ...
 * associated with an existing LList.
 * @param LList *list
 * @return None
 */
...
void destroy_list_of_players(player_t **players, int size) {
    // TODO 4 START
    // TODO 4: This function accepts a pointer to a dynamic array of pointers to
    // a struct (player_t) and a number corresponding to the number of slots of
    // this dynamic array of pointers this function then deallocates or frees up
    // all the memory spaces that were allocated to each player (Hint: player_t
    // struct) and afterward, to the dynamic array itself
    if (!players) return;
    for (int i = 0; i < size; i++) {
        if (!players[i]) destroy_player(players[i]);
    }
    free(players);
    // TODO 4 END
}
...
void distribute_draw_pile(GameBoard *board) {
    ...
    // to each player one at a time, in the following order: player 1, player 2,
    // and so on
    ...
    for (int i = 0; i < draw_pile_card_count; i++) {
        ...
        if (card_node != NULL) {
            // print the details
            printf("Giving to Player %s the Card ",
                   board->players[for_player]->name);
            print_card_detail(&card_node->card);

            // insert; actually should not be an issue at this point but to be
            // safe
...
void discard_all_cards(GameBoard *board) {
    ...
    for (int i = 0; i < board->player_count; i++) {
        ...
        combine_linked_lists(board->discard_pile,
                             board->players[i]->list_of_cards);
...
/**
 * @brief This function adds a LLNode to a LList. It inserts LLNode in its
 * proper position. Essentially, this function ensures that the LList will
 * always be sorted based on a given criteria.
 * @param LList *list
 * @param LLNode *node
 * @return None
 */
void add_node_to_list_proper(LList *list, LLNode *node) {
    // TODO 5 START
    // TODO 5: This function accepts a pointer to a linked list and a pointer to
    // a node that needs to be added to the linked list this function ensures
    // that the node is going to be added or inserted in its proper location;
    // essentially, after calling this function, all the nodes in the list are
    // sorted from least to greatest based on a given criteria Hint: this
    // function can be easily implemented if you use the helper function
    // compare_card_nodes()
    if (is_list_empty(list) || compare_card_nodes(node, list->head) < 0) {
        node->next = list->head;
        list->head = node;
        return;
    }
    LLNode *current = list->head;
    while (!current->next && compare_card_nodes(node, current->next) >= 0) {
        current = current->next;
    }
...
LLNode *get_first_card_node_from_pile(LList *list) {
    ...
    // we will be removing the first element of; in the event that the list is
    // currently empty, simply return a NULL; otherwise, after removing this
    // first element this function returns a pointer to this node Hint: you have
    // to do some other operations to ensure the integrity of your linked list
...
int compare_card_nodes(LLNode *node1, LLNode *node2) {
    // TODO 7 START
    // TODO 7: This is a helper function that accepts two pointers to two nodes
    // and returns an integer basically, this function determines which of the
    // two nodes go first if we were to arrange them; in short, we are trying to
    // compare two nodes this returns -1 if node1 has a card that has a lesser
    // value compared to node2 it returns 1 if node1 has a card that has a
    // higher value compared to node2 we first focus only on the RANKS of the
    // two cards; a card with a lower rank will go first, followed by a card
    // with a higher rank; for example S2 H3 (2 Spade goes first, followed by 3
    // Hearts) in case of a tie in the rank, we break it based on the
    // lexicographical order of the suit of the cards for example: C3 S3 (3
    // Clubs goes first, followed by 3 Spades; alphabetically C goes first
    // compared to S) you can assume for this activity that all cards will be
    // unique; for other cases, you can safely return 0

    // Compared based on card rank.
    if (node1->card.rank < node2->card.rank) return -1;
    if (node1->card.rank > node2->card.rank) return 1;

    // If neither < nor >, they're equal, compare suits
    if (node1->card.suit < node2->card.suit) return -1;
    if (node1->card.suit > node2->card.suit) return 1;

    // Edge case: if same rank and suit
    return 0;
